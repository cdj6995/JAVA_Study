...
public class BookList extends Books implements Book {
	...
	public void rent() {
		...
		for(int i= 0; i < booklist.size(); i++) {
			if(booklist.get(i).get(4).equals("대여 가능")) {
				System.out.printf("%d번 책 제목 : %s, 책 번호 : %s %s \n", ++pcnt, booklist.get(i).get(0), booklist.get(i).get(3), booklist.get(i).get(4));
			}
		}
		...
		for(int i = 0; i < booklist.size(); i++) {
			if(booklist.get(i).get(4).equals("대여 가능")) {
				++pcnt;
				if(num == pcnt) {
					System.out.printf("\n %s (%s) 를 대여합니다. \n", booklist.get(i).get(0), booklist.get(i).get(3));
		...
		try {
			...
			for(int i = 0; i < booklist.size(); i++) {
				System.out.printf("책 제목 : %s, 책 저자 : %s, 책 종류 : %s, 책 번호 : %s, %s \n",
						booklist.get(i).get(0),booklist.get(i).get(1),booklist.get(i).get(2),booklist.get(i).get(3),booklist.get(i).get(4));
				
				dos.writeUTF(booklist.get(i).get(0) + " / " + booklist.get(i).get(1) + " / " + booklist.get(i).get(2) + " / " + booklist.get(i).get(3) + " / " + booklist.get(i).get(4) + "\n");
			}
	...
	public void rentPossible() {
		...
		for(int i= 0; i < booklist.size(); i++) {
			if(booklist.get(i).get(4).equals("대여 가능")) {
				System.out.printf("책 제목 : %s, 책 번호 : %s %s \n", booklist.get(i).get(0), booklist.get(i).get(3), booklist.get(i).get(4));
				pcnt++;
			}
		}
